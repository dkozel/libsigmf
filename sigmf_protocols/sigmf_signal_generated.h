...
struct signal_detail FLATBUFFERS_FINAL_CLASS : private ::flatbuffers::Table {
  ...
  bool Verify(::flatbuffers::Verifier &verifier) const {
    return VerifyTableStart(verifier) &&
           ...
           VerifyField<double>(verifier, VT_CHANNEL_BW, 8) &&
           VerifyField<uint64_t>(verifier, VT_CHANNEL, 8) &&
...
struct signal_emitterT : public ::flatbuffers::NativeTable {
  ...
  signal_emitterT() = default;
  signal_emitterT(const signal_emitterT &o);
  signal_emitterT(signal_emitterT&&) FLATBUFFERS_NOEXCEPT = default;
  signal_emitterT &operator=(signal_emitterT o) FLATBUFFERS_NOEXCEPT;
};
...
struct signal_emitter FLATBUFFERS_FINAL_CLASS : private ::flatbuffers::Table {
  ...
  bool Verify(::flatbuffers::Verifier &verifier) const {
    return VerifyTableStart(verifier) &&
           VerifyField<uint64_t>(verifier, VT_SEID, 8) &&
           VerifyOffset(verifier, VT_MANUFACTURER) &&
           verifier.VerifyString(manufacturer()) &&
           VerifyField<double>(verifier, VT_POWER_TX, 8) &&
           VerifyField<double>(verifier, VT_POWER_EIRP, 8) &&
...
inline signal_emitterT::signal_emitterT(const signal_emitterT &o)
      : seid(o.seid),
        manufacturer(o.manufacturer),
        power_tx(o.power_tx),
        power_eirp(o.power_eirp),
        geolocation((o.geolocation) ? new sigmf::core::geojson_pointT(*o.geolocation) : nullptr) {
}

inline signal_emitterT &signal_emitterT::operator=(signal_emitterT o) FLATBUFFERS_NOEXCEPT {
  std::swap(seid, o.seid);
  std::swap(manufacturer, o.manufacturer);
  std::swap(power_tx, o.power_tx);
  std::swap(power_eirp, o.power_eirp);
...
inline AnnotationT::AnnotationT(const AnnotationT &o)
      : detail((o.detail) ? new sigmf::signal::signal_detailT(*o.detail) : nullptr),
        emitter((o.emitter) ? new sigmf::signal::signal_emitterT(*o.emitter) : nullptr) {
}

inline AnnotationT &AnnotationT::operator=(AnnotationT o) FLATBUFFERS_NOEXCEPT {
  std::swap(detail, o.detail);
  std::swap(emitter, o.emitter);
  return *this;
}
...
inline void Annotation::UnPackTo(AnnotationT *_o, const ::flatbuffers::resolver_function_t *_resolver) const {
  (void)_o;
  (void)_resolver;
  { auto _e = detail(); if (_e) { if(_o->detail) { _e->UnPackTo(_o->detail.get(), _resolver); } else { _o->detail = std::shared_ptr<sigmf::signal::signal_detailT>(_e->UnPack(_resolver)); } } else if (_o->detail) { _o->detail.reset(); } }
  { auto _e = emitter(); if (_e) { if(_o->emitter) { _e->UnPackTo(_o->emitter.get(), _resolver); } else { _o->emitter = std::shared_ptr<sigmf::signal::signal_emitterT>(_e->UnPack(_resolver)); } } else if (_o->emitter) { _o->emitter.reset(); } }
}
...
inline DescrT::DescrT(const DescrT &o)
      : global((o.global) ? new sigmf::signal::GlobalT(*o.global) : nullptr),
        annotation((o.annotation) ? new sigmf::signal::AnnotationT(*o.annotation) : nullptr),
        capture((o.capture) ? new sigmf::signal::CaptureT(*o.capture) : nullptr) {
}
...
inline void Descr::UnPackTo(DescrT *_o, const ::flatbuffers::resolver_function_t *_resolver) const {
  (void)_o;
  (void)_resolver;
  { auto _e = global(); if (_e) { if(_o->global) { _e->UnPackTo(_o->global.get(), _resolver); } else { _o->global = std::shared_ptr<sigmf::signal::GlobalT>(_e->UnPack(_resolver)); } } else if (_o->global) { _o->global.reset(); } }
  { auto _e = annotation(); if (_e) { if(_o->annotation) { _e->UnPackTo(_o->annotation.get(), _resolver); } else { _o->annotation = std::shared_ptr<sigmf::signal::AnnotationT>(_e->UnPack(_resolver)); } } else if (_o->annotation) { _o->annotation.reset(); } }
  { auto _e = capture(); if (_e) { if(_o->capture) { _e->UnPackTo(_o->capture.get(), _resolver); } else { _o->capture = std::shared_ptr<sigmf::signal::CaptureT>(_e->UnPack(_resolver)); } } else if (_o->capture) { _o->capture.reset(); } }
}
